...
const { notesLoggerMobileDevelopment } = require("../../../controller/logger")
...
server.get("/lokasi",middleKey,key,middleLocationKeyword, async (req, res)=>{ 
  ...
  notesLoggerMobileDevelopment(`/lokasi`, `Pencarian lokasi menggunakan keyword ${keyword}`, key )
...
server.get("/lokasi/negara",middleKey,key,middleLocationCountries, async (req, res)=>{    
  ...
  notesLoggerMobileDevelopment(`/lokasi/negara`, `Pencarian detail bedasarkan lokasi negara dengan id ${bod?.id}`, key)
  if(resNet?.data?.data?.length === 0){
...
server.get("/lokasi/provinsi",middleKey,key,middleLocationProvinces, async (req, res)=>{    
  ...
  notesLoggerMobileDevelopment(`/lokasi/provinsi`, `Pencarian detail bedasarkan lokasi provinsi dengan id ${bod?.id}`, key)
  if(resNet?.data?.data?.length === 0){
...
server.get("/lokasi/kota",middleKey,key,middleLocationCities, async (req, res)=>{    
  ...
  notesLoggerMobileDevelopment(`/lokasi/kota`, `Pencarian detail bedasarkan lokasi kota dengan id ${bod?.id}`, key)
  if(resNet?.data?.data?.length === 0){
...
server.get("/lokasi/kecamatan",middleKey,key,middleLocationSub, async (req, res)=>{    
  ...
  notesLoggerMobileDevelopment(`/lokasi/kecamatan`, `Pencarian detail bedasarkan lokasi kecamatan dengan id ${bod?.id}`, key)
  if(resNet?.data?.data?.length === 0){
...
server.get("/kurir/tipe",middleKey,key, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/kurir/tipe`, `Request untuk mendapatkan list tipe service`, key)
  if(data?.length === 0){
...
server.get("/kurir/tipe/select",middleKey,key, async (req, res)=>{
  const key = req?.key
  const data = await getTipeService()
  notesLoggerMobileDevelopment(`/kurir/tipe/select`, `Request untuk mendapatkan list tipe service dengan mode select`, key)  
  const arr=[]
...
server.get("/paket/tipe",middleKey,key, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/paket/tipe`, `Request untuk mendapatkan list tipe paket`, key)
  if(data?.length === 0){
    sender(res, 404, `Data tipe  tidak ditemukan`, [])
  }else{
...
server.get("/paket/tipe/select",middleKey,key, async (req, res)=>{
  const key = req?.key
  const data = await getTipePackage()
  notesLoggerMobileDevelopment(`/paket/tipe/select`, `Request untuk mendapatkan list tipe paket dengan mode select`, key)
  const arr=[]
  const len = data?.length
  for(let i=0;i<len;i++){
...
server.get("/order/:order_id",middleKey,key, middleOrderDetail, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/order/:order_id`, `Request untuk mendapatkan detail order dengan id ${id}`, key)
...
server.delete("/order/cancel",middleKey,key, json, middleCancelOrder, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/order/cancel`, `Cancel order dengan id ${bod?.order_id}`, key)
  if(resNet?.data?.data === null){
...
server.delete("/pickup/cancel",middleKey,key, json, middleCancelPickup, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/pickup/cancel`, `Cancel pickup dengan id ${bod?.pickup_code}`, key)
  if(resNet?.data?.data === null){
...
server.post("/pickup",middleKey,key, json, middleCreatePickup, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/pickup`, `Pembuatan pickup bedasarkan order`, key)
  if(resNet?.data?.data === null){
...
server.post("/order",middleKey,key, json, middleOrderNew, async (req, res)=>{  
  ...
  notesLoggerMobileDevelopment(`/order`, `Pembuatan order bedasarkan barang item pembelian`, key)
  if(resNet?.data === null){
...
server.post("/kurir/domestik",middleKey,key, json, middleKurirDomestik, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/kurir/domestik`, `Mendapatkan list kurir bedasarkan rekomendasi jarak yang ditentukan`, key)
...
server.post("/kurir/domestik/:tipe",middleKey,key, middleKurirDomestikTipe, json, middleKurirDomestik, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/kurir/domestik/:tipe`, `Mendapatkan list kurir bedasarkan tipe ${tipe}`, key)
  if(resNet?.data?.data?.pricings?.length === 0){
    sender(res, 404, "Data kurir domestik kosong", [])  
  }else if(resNet?.data?.data === null || resNet?.data?.data === undefined){
    sender(res, 400, "Terjadi kesalahan", resNet?.data)  
...
server.post("/kurir/internasional",middleKey,key, json, middleKurirInternasional, async (req, res)=>{
  ...
  notesLoggerMobileDevelopment(`/kurir/internasional`, `Mendapatkan list kurir bedasarkan rekomendasi jarak (internasional)`, key)
  if(resNet?.data?.data?.pricings?.length === 0){
